...
│   │       └── config.ini          источник, ключ, предыдущий снимок, порог длины имени
│   └── latest/
│       ├── image/                  актуальная копия, в неё пишет rsync
│       └── date_last_backup.txt    дата последнего запуска, по ней run.sh не повторяет копирование
├── docs/
│   ├── README.md
│   └── structure.md
├── logs/
...
- Если за ночь ничего не изменилось, снимок не создаётся — обновляется только `date_last_backup.txt`.
